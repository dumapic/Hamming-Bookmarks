...
public class StringUtils
{
	...
	public static int indexOf(String str, String searchStr, int startPos)
	{
		...
		if (searchStr.length() == 0 && startPos >= str.length()) { return str
				.length(); }
	...
	{
		if (isEmpty(str) || ArrayUtils.isEmpty(searchChars)) { return INDEX_NOT_FOUND; }
			...
			for (int j = 0; j < searchLen; j++)
			{
				if (searchChars[j] == ch)
				{
							...
							&& CharUtils.isHighSurrogate(ch))
					{
						...
						if (searchChars[j + 1] == str.charAt(i + 1)) { return i; }
					}
					else
					{
						return i;
	...
	{
		if (isEmpty(str) || isEmpty(searchChars)) { return INDEX_NOT_FOUND; }
	...
	{
		if (isEmpty(str) || ArrayUtils.isEmpty(searchChars)) { return false; }
			...
			for (int j = 0; j < searchLength; j++)
			{
				if (searchChars[j] == ch)
				{
					if (CharUtils.isHighSurrogate(ch))
					{
						...
						if (i < csLast
								&& searchChars[j + 1] == str.charAt(i + 1)) { return true; }
					}
					else
					{
	...
	public static int indexOfAnyBut(String str, String searchChars)
	{
		...
		for (int i = 0; i < strLen; i++)
		{
			...
			if (i + 1 < strLen && CharUtils.isHighSurrogate(ch))
			{
				...
				if (chFound && searchChars.indexOf(ch2) < 0) { return i; }
			}
			else
			{
				if (!chFound) { return i; }
	...
	{
		if (str == null || searchChars == null) { return true; }
		int csLen = str.length();
		int csLast = csLen - 1;
		int searchLen = searchChars.length;
		int searchLast = searchLen - 1;
			...
			for (int j = 0; j < searchLen; j++)
			{
				if (searchChars[j] == ch)
				{
					if (CharUtils.isHighSurrogate(ch))
					{
						...
						if (i < csLast
								&& searchChars[j + 1] == str.charAt(i + 1)) { return false; }
					}
					else
					{
						// ch is in the Basic Multilingual Plane
	...
	public static String substringBetween(String str, String open, String close)
	{
		...
		if (start != INDEX_NOT_FOUND)
		{
			...
			if (end != INDEX_NOT_FOUND) { return str.substring(
					start + open.length(), end); }
		}
		return null;
	}
	...
	{
		if (isEmpty(str) || isEmpty(remove)) { return str; }
		if (startsWithIgnoreCase(str, remove)) { return str.substring(remove
				.length()); }
		return str;
	}
	...
	{
		if (isEmpty(str) || isEmpty(remove)) { return str; }
		if (str.endsWith(remove)) { return str.substring(0, str.length()
				- remove.length()); }
		return str;
	}
	...
	{
		if (isEmpty(str) || isEmpty(remove)) { return str; }
		if (endsWithIgnoreCase(str, remove)) { return str.substring(0,
			...
			String[] replacementList, boolean repeat, int timeToLive)
	{
		...
		if (timeToLive < 0) { throw new IllegalStateException("TimeToLive of "
	...
	{
		if (isEmpty(str) || separator == null) { return str; }
		if (str.endsWith(separator)) { return str.substring(0, str.length()
				- separator.length()); }
		return str;
	}
	...
	public static String repeat(String str, int repeat)
	{
		...
		if (inputLength == 1 && repeat <= PAD_LIMIT) { return padding(repeat,
				str.charAt(0)); }
	...
	private static String padding(int repeat, char padChar)
			throws IndexOutOfBoundsException
	{
		if (repeat < 0) { throw new IndexOutOfBoundsException(
	...
	public static String rightPad(String str, int size, char padChar)
	{
		...
		if (pads > PAD_LIMIT) { return rightPad(str, size,
	...
	public static String leftPad(String str, int size, char padChar)
	{
		...
		if (pads > PAD_LIMIT) { return leftPad(str, size,
				String.valueOf(padChar)); }
	...
	{
		if (str == null || isEmpty(str)) { return false; }
		int sz = str.length();
		for (int i = 0; i < sz; i++)
		{
			if (Character.isLowerCase(str.charAt(i)) == false) { return false; }
		}
		return true;
	}
	...
	public static boolean isAllUpperCase(String str)
	{
		...
		for (int i = 0; i < sz; i++)
		{
			if (Character.isUpperCase(str.charAt(i)) == false) { return false; }
		}
		return true;
	}
	...
	{
		if (str1 == str2) { return INDEX_NOT_FOUND; }
	...
	{
		if (strs == null || strs.length <= 1) { return INDEX_NOT_FOUND; }
	...
	{
		if (isEmpty(string) || ArrayUtils.isEmpty(searchStrings)) { return false; }
	...
	public static boolean endsWithAny(String string, String[] searchStrings)
	{
		...
		{
			String searchString = searchStrings[i];
			if (StringUtils.endsWith(string, searchString)) { return true; }
		}
		return false;
	}
